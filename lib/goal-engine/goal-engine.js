...
import { sendMilestoneWebhook, dispatchMilestoneWebhookAction } from './outbound-webhook.js';
 ...
 * @param {Object|null} [params.fxProvider] - Optional FX provider
 * @param {Object|null} [params.executionCtx] - Optional Cloudflare Worker execution context
...
export async function processRevenueEventForGoalEngine({
  ...
  const contribResult = await database.addGoalContribution(workspaceId, {
    ...
    fxProvider: evaluation.fxProvider,
    fxTimestamp: evaluation.fxTimestamp,
    fxPair: evaluation.fxPair,
    ...
    metadata: {
      ...
      fx: {
        rate: evaluation.fxRate,
        provenance: evaluation.fxRateProvenance,
        provider: evaluation.fxProvider,
        timestamp: evaluation.fxTimestamp,
        pair: evaluation.fxPair
      }
    }
  });
    ...
    if (triggerRecord.triggered) {
      ...
      if (milestone.webhookActionUrl) {
        dispatchMilestoneWebhookAction({
          ...
          workspaceId,
          goalId: activeGoal.id,
          triggerId: triggerRecord.triggerRecord?.id || null,
