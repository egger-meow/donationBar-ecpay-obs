  ...
  async createGoal(workspaceId, goalData = {}) {
    ...
    if (this.isProduction && this.connected) {
      ...
      try {
        ...
        if (isActive) {
          await client.query('SELECT 1 FROM user_workspaces WHERE id = $1 FOR UPDATE', [workspaceId]);
          await client.query('UPDATE goals SET is_active = FALSE WHERE workspace_id = $1 AND is_active = TRUE', [workspaceId]);
        }
  ...
  async addGoalContribution(workspaceId, contributionData = {}) {
    ...
    if (this.isProduction && this.connected) {
      ...
      try {
        ...
        const insertResult = await client.query(`
          INSERT INTO goal_contributions (
            ...
            source_tier, rule_type, fx_rate, fx_rate_provenance, fx_provider,
            fx_timestamp, fx_pair, contribution_minor, currency, supporter_name,
            message, is_synthetic, reason, metadata
          )
        ...
        `, [
          ...
          contributionData.fxProvider ?? 'system',
          contributionData.fxTimestamp || new Date(),
          contributionData.fxPair || `${goalRow.display_currency}/${goalRow.display_currency}`,
          contributionMinor,
      ...
      const contrib = {
        ...
        fxProvider: contributionData.fxProvider ?? 'system',
        fxTimestamp: contributionData.fxTimestamp ?? new Date().toISOString(),
        fxPair: contributionData.fxPair ?? `${goal.displayCurrency}/${goal.displayCurrency}`,
        contributionMinor,
