...
/**
 * Resolves the default FX provider based on environment.
 * Production/Staging: null (requires explicit production provider integration or fails loud with FX_RATE_UNAVAILABLE).
 * Sandbox/Test/Dev: FixtureFxProvider (permitted for offline testing and deterministic fixtures).
 */
export function getDefaultFxProvider(env = process.env) {
  if (isProduction(env)) {
    return null;
  }
  return new FixtureFxProvider();
}

/**
 * Global active FX provider reference.
 */
let activeFxProvider = getDefaultFxProvider();
...
export function resetFxProvider(env = process.env) {
  activeFxProvider = getDefaultFxProvider(env);
}
