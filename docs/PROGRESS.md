...
- **FX Provider Architecture & Bounded Precision:** Replaced hardcoded market exchange rates as silent production defaults. Added environment-aware default provider resolution (`getDefaultFxProvider()`) returning `null` in staging/production (failing loud with `FX_RATE_UNAVAILABLE` on unconfigured cross-currency donations) while allowing `FixtureFxProvider` in test/sandbox. Same-currency ($1:1$, provider `system`) and creator custom overrides (`custom_override`, provider `creator_override`) always resolve without external providers. Fixed `fxProvider` parameter propagation in `processRevenueEventForGoalEngine` through to `evaluateGoalRule`. Persisted full snapshot columns and JSON metadata (`fx_rate`, `fx_rate_provenance`, `fx_provider`, `fx_timestamp`, `fx_pair`) on every contribution record in PostgreSQL and JSON storage, ensuring complete historical immutability when provider rates later change. Documented IEEE-754 bounded half-up integer rounding semantics.
