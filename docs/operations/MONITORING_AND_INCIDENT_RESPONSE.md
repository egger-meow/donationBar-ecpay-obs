...
- **Fixed vocabulary:** only `readiness_check_failed`, `payment_webhook_invalid_merchant`, `payment_webhook_invalid_signature`, `payment_webhook_decryption_failed`, `payment_webhook_unexpected_error`, `subscription_initial_callback_failed`, `subscription_callback_unexpected_error`, and `http_unhandled_error` are ever delivered. Any other event name passed to `sendAlert` is silently dropped before a network call is made.
