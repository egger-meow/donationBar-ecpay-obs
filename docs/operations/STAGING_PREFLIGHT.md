...
3. Sends two `GET` requests, and only these two: `/health/live` and `/health/ready` ([server.js:57-64](../../server.js#L57)). Nothing else is called — no database, no payment provider, no OAuth endpoint.
...
[test/staging-preflight.test.js](../../test/staging-preflight.test.js) covers: missing/malformed base URL, the HTTPS-outside-sandbox rejection, that exactly the two health endpoints are called by default, pass/fail reporting for a `503` readiness response, that the alert-webhook check is skipped unless explicitly enabled, and that the webhook URL never appears in returned diagnostics.
